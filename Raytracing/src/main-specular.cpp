...
int main()
{
    ...
    m.metallic = 0.6;
    m.roughness = 0.1;
    readObj("sphere-spe.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -1.5, 0), vec3(1, 1, 1)), true);
    m.metallic = 0.0;
    m.roughness = 0.0;
    ...
    m.metallic = 0.3;
    m.roughness = 0.1;
    readObj("sphere-spe.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(-2, -1.5, 0), vec3(1, 1, 1)), true);
    m.metallic = 0.0;
    m.roughness = 0.0;   
    ...
    //m.baseColor = vec3(1, 1, 1);
    //m.emissive = vec3(30, 20, 10);
    //readObj("Light-specular.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -0.4, 0), vec3(1, 1, 1)), false);
    ...
    bool r = HDRLoader::load("indoor-hdr2.hdr", hdrRes);
